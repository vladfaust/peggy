...
import inferenceMatchResult from "./inference-match-result.js"
...
function generateBytecode(ast) {
  ...
  function buildCondition(match, condCode, thenCode, elseCode) {
    if (match === inferenceMatchResult.ALWAYS_MATCH) { return thenCode; }
    if (match === inferenceMatchResult.NEVER_MATCH)  { return elseCode; }
  ...
  const generate = visitor.build({
    ...
    named(node, context) {
      ...
      const nameIndex = (match === inferenceMatchResult.NEVER_MATCH)
        ? null
    ...
    choice(node, context) {
      function buildAlternativesCode(alternatives, context) {
        ...
        if (match === inferenceMatchResult.ALWAYS_MATCH) {
          return first;
        }
        ...
        return buildSequence(
          ...
          alternatives.length > 1
            ? buildCondition(
              inferenceMatchResult.SOMETIMES_MATCH,
              [op.IF_ERROR],
              buildSequence(
                [op.POP],
    ...
    action(node, context) {
      ...
      const functionIndex = emitCall && match !== inferenceMatchResult.NEVER_MATCH
    ...
    literal(node) {
      if (node.value.length > 0) {
        ...
        const needConst = match === inferenceMatchResult.SOMETIMES_MATCH
                      || (match === inferenceMatchResult.ALWAYS_MATCH && !node.ignoreCase);
        ...
        const expectedIndex = (match !== inferenceMatchResult.ALWAYS_MATCH)
          ? addExpectedConst({
            type: "literal",
            value: node.value,
            ignoreCase: node.ignoreCase,
    ...
    class(node) {
      ...
      const classIndex = match === inferenceMatchResult.SOMETIMES_MATCH ? addClassConst(node) : null;
      // Expectation not required if node always match
      const expectedIndex = (match !== inferenceMatchResult.ALWAYS_MATCH)
        ? addExpectedConst({
          type: "class",
